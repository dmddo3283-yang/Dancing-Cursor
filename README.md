...
- **탭을 캡처하지 않습니다.** content script가 페이지의 미디어 요소(`<video>`/`<audio>`)를 `captureStream()`으로 비침습적으로 tap 해 Web Audio `AnalyserNode`로 분석합니다.
- 이 방식은 `tabCapture`를 쓰지 않으므로 **Dancing Chrome의 탭 캡처와 충돌하지 않아 두 확장을 동시에 켤 수 있습니다.**
- Dancing Chrome과 같은 `BeatDetector` 알고리즘으로 음량·저음·비트를 계산해 마우스를 따라다니는 미러볼을 그립니다.
...
1. 툴바의 Mirrorball Cursor 아이콘을 클릭하고 가운데 **START** 버튼을 눌러 켭니다. (선택 창·추가 탭 없음)
2. 탭에서 음악(또는 영상)을 재생합니다.
3. 마우스를 움직이면 소리가 나는 동안 커서가 미러볼로 바뀌며 빛을 뿜습니다.
4. 끄려면 **STOP** 버튼을 누르거나 단축키 **Alt+Shift+M** (켜기·끄기 모두 가능).

> Dancing Chrome과 **동시에** 켜도 됩니다. 이 확장은 탭을 캡처하지 않고 페이지의 미디어 소리를 tap 하므로 서로 충돌하지 않습니다.
